...
public function get_booking_dt(Request $req) {
    ...
    if ($get_data->isEmpty()) {
        // return array("StatusResult" => "failure");
         ...
         return response()->json($this->encryption($data));
...
$mobileno2='9177385289';
            ...
            $returnsuccessdata = $this->encryption(array("StatusResult" => "success","id" => $b_id));
...
public function document_upload_lh_scre(Request $req, $encryptedData)
{
   ...
   //  return $b_id;
    ...
    if ($req->hasFile('lhscrev')) {
        $loc = "uploads";
        $file = $req->file('lhscrev');
        $lhscrev= date("His") . rand(11111, 99999) . '.' . $file->getClientOriginalExtension();
      $filePath = $file->move($loc, $lhscrev);
