...
function unlock_dt(Request $req){
    ...
    if ($deletedRows) {
        $returnsuccessdata =  encryption(array("status"=>"success"));

              return  array("return_response"=>$returnsuccessdata);
    } else {
         $returnsuccessdata =  encryption(array("status"=>"failure"));
          return  array("return_response"=>$returnsuccessdata);
    }
...
public function lock_ins_dt(Request $req)
{
    ...
    $dataArray = json_decode($jsonString, true);
        ...
        $tbl = DB::table('lock_table')->insert($rowsToInsert);
         ...
         $returnsuccessdata =  encryption(array("status" => "error", "message" => $e->getMessage()));
