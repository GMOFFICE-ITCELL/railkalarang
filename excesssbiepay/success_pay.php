...
// DB_PASSWORD=alMZlNV_Z?*p
    ...
    // Prepare and bind the SQL statement
      $stmt = $conn->prepare("UPDATE excess_Transaction_table SET merchant_id = ?, merchant_order_no = ?,transaction_success_response = ?,transaction_status = ?,level = ? WHERE Order_number = ?");
    ...
    $idmt = $conn->prepare("SELECT Ref_id FROM excess_Transaction_table WHERE merchant_id = ?");
...
$mlevel = "1";
$status = "paid";
 $lmt = $conn->prepare("update Booking_Form set excess_level = ?, excess_merchant_order_no = ?,excess_status = ? where BF_id = ?");
...
$lmt->close();
    ...
    $stmt = $conn->prepare("UPDATE excess_Transaction_table SET dv_response = ?,dv_success_id = ? WHERE Order_number = ?");
    ...
    //   header("Location: http://localhost:4200/#/excessreciept");
      
        header("Location: https://rlyfunhalls.indianrailways.gov.in/Railkalarang/#/excessreciept");
} else {
